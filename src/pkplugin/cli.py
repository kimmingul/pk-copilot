...
def _parse_init_params(raw: str) -> dict[str, float]:
    ...
    for token in raw.split(","):
        ...
        try:
            ...
            raise argparse.ArgumentTypeError(f"Cannot parse value for {name!r}: {value!r}") from exc
    return params
...
def _cmd_nca(args: argparse.Namespace) -> int:
    ...
    if args.config:
        ...
        if not cfg_path.is_file():
            print(
                json.dumps({"status": "error", "error": f"Config file not found: {cfg_path}"}),
                file=sys.stderr,
            )
            return 1
...
def _cmd_keygen(args: argparse.Namespace) -> int:
    ...
    if args.passphrase:
        ...
        if pw != pw2:
            print(
                json.dumps({"status": "error", "error": "Passphrases do not match"}),
                file=sys.stderr,
            )
            return 1
...
def _build_parser() -> argparse.ArgumentParser:
    ...
    p_pdfit.add_argument(
        "pd_dataset", metavar="pd_dataset.csv", help="PD dataset CSV (time, concentration, effect)."
    )
    ...
    p_report.add_argument(
        "--format", choices=["html", "pdf"], default="html", help="Report format (default: html)."
    )
    ...
    p_compare.add_argument(
        "--backend",
        choices=["pknca", "noncompart"],
        default="pknca",
        help="Reference backend (default: pknca).",
    )
    ...
    p_keygen.add_argument(
        "--output",
        required=True,
        metavar="keys/myuser.key",
        help="Output path for private key PEM.",
    )
    p_keygen.add_argument(
        "--passphrase", action="store_true", help="Prompt for passphrase to encrypt private key."
    )
    p_keygen.add_argument(
        "--force", action="store_true", help="Overwrite existing key file if it already exists."
    )
    ...
    p_sign.add_argument(
        "--identity", required=True, metavar="user@example.com", help="Signer identifier."
    )
    p_sign.add_argument(
        "--meaning",
        required=True,
        choices=["authored", "reviewed", "approved", "rejected"],
        help="Signature meaning.",
    )
    p_sign.add_argument(
        "--key", required=True, metavar="keys/myuser.key", help="Path to Ed25519 private key."
    )
    p_sign.add_argument(
        "--passphrase", default=None, metavar="PASS", help="Key passphrase (if encrypted)."
    )
    p_sign.add_argument(
        "--auth-token",
        default="",
    ...
    p_lock.add_argument(
        "--locked-by", default=None, dest="locked_by", help="Identifier of who is locking."
    )
    p_lock.add_argument(
        "--require-signatures",
        default=None,
        dest="require_signatures",
        metavar="authored,reviewed,approved",
    ...
    p_vchain.add_argument(
        ...
        dest="chain_dir",
        metavar="dir/",
        help="Chain directory (default: audit_dir).",
    )
