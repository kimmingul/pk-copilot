...
|---|---|---|---|---|
| AUC 적분 기본 | `linear` (WNL 5.3 p.196) | `linear` (WNL 6.4 UG §7.2.3) | `linear` (WNL 8.3 UG §8.2.1) | `linear` |
| Lambda_z 기본 | Best Fit (WNL 5.3 p.196) | Best Fit | Best Fit | Best Fit |
| Best Fit tolerance (Adj R²) | 0.0001 (WNL 5.3 p.196) | 0.0001 | 0.0001 | 0.0001 |
| C0 (IV bolus) 추정 | log back-extrap (WNL 5.3 p.196) | log back-extrap | log back-extrap | log back-extrap |
| AUCINF_pred 출력 | 지원 (WNL 5.3 Table B-4) | 지원 | 지원 | 지원 |
| Span ratio 경고 임계 | < 1.5 (plugin-internal default) | < 1.5 | < 1.5 | < 1.5 |
| Partial AUC 기본 보간 | `linear` | `linear` | `linear` | `linear` |
| BLOQ pre-dose 처리 | 0 (WNL 5.3 p.195) | 0 | 0 | 0 |
| BLOQ embedded 처리 | missing | missing | missing | missing |
| BLOQ trailing 처리 | exclude | exclude | exclude | exclude |
| MRT (IV infusion) 정의 | `AUMC/AUC` (WNL 5.3 p.199) | `AUMC/AUC - T_inf/2` | `AUMC/AUC - T_inf/2` | `AUMC/AUC - T_inf/2` |
| Vss 정의 | `CL · MRT` | `CL · MRT` | `CL · MRT` | `CL · MRT` |
| AUC %Extrap 경고 임계 | 20% (FDA/EMA convention) | 20% | 20% | 20% |
...
| `Tlast` | ✓ | ✓ | ✓ | |
| `Clast` | ✓ | ✓ | ✓ | |
| `Clast_pred` | ✓ (WNL 5.3 Table B-4) | ✓ | ✓ | |
| `C0` | log back-extrap (WNL 5.3 p.196) | log back-extrap | log back-extrap | |
| `AUCINF_obs` | ✓ | ✓ | ✓ | |
| `AUCINF_pred` | ✓ (WNL 5.3 Table B-4) | ✓ | ✓ | |
| `AUC_%Extrap_obs` | ✓ | ✓ | ✓ | |
| `AUC_%Extrap_pred` | ✓ | ✓ | ✓ | |
| `Lambda_z_lower` | ✓ | ✓ | ✓ | |
| `Lambda_z_upper` | ✓ | ✓ | ✓ | |
| `No_points_Lambda_z` | ✓ | ✓ | ✓ | capital L (WNL output column name) |
| `Rsq_adjusted` | ✓ | ✓ | ✓ | |
| `Span` | ✓ | ✓ | ✓ | time span in hours |
| `Span_ratio` | ✓ | ✓ | ✓ | ratio Span/t½ (plugin-internal; WNL uses `Span`) |
| `AUMCINF_obs` / `AUMCINF_pred` | both | both | both | |
| `MRTINF_obs` / `MRTINF_pred` | both | both | both | |
| `Vss_obs` | ✓ | ✓ | ✓ | `CL · MRTINF_obs` |
| `Vss_pred` | ✓ | ✓ | ✓ | `CL · MRTINF_pred` |
...
| 항목 | 5.3 | 6.4 | 8.3 | 비고 |
|---|---|---|---|---|
| df 계산 (mixed model) | Satterthwaite | Satterthwaite | Satterthwaite (WNL 8.3 UG §BE; Kenward-Roger 없음) | |
...
| 항목 | 5.3 | 6.4 | 8.3 | 비고 |
|---|---|---|---|---|
| Weighting 기본 | `uniform` (WNL 5.3 §10) | `1/y²` (WNL 6.4 UG §10) | `1/y²` | |
| AIC 정의 | `N·ln(WRSS) + 2P` | `N·ln(WRSS) + 2P` | `N·ln(WRSS) + 2P` (WNL 8.3 UG p.547) | |
| BIC 정의 | `N·ln(WRSS) + P·ln(N)` | 동일 | 동일 | |
...
DEFAULTS: dict[WNVersion, dict] = {
    WNVersion.V5_3: {
        "auc_method": "linear",           # WNL 5.3 p.196 confirmed
        "lambda_z_method": "best_fit",    # WNL 5.3 p.196
        "lambda_z_tolerance": 0.0001,     # WNL 5.3 p.196
        "c0_method": "log_back_extrap",   # WNL 5.3 p.196 (same algorithm as 6.4/8.3)
        "output_pred_variants": True,     # WNL 5.3 Table B-4 confirms pred variants
        "bloq_policy": {
            "pre_dose": "zero", "up_leading": "zero",
            "embedded": "missing", "trailing": "exclude",
        },
        "comp_weighting_default": "uniform",  # WNL 5.3 §10
    },
    WNVersion.V6_4: {
        "auc_method": "linear",           # WNL 6.4 UG §7.2.3 confirmed
        ...
        "comp_weighting_default": "1_over_y_squared",  # WNL 6.4 UG §10
    },
    WNVersion.V8_3: {
        "auc_method": "linear",           # WNL 8.3 UG §8.2.1 confirmed
        "lambda_z_method": "best_fit",
        "lambda_z_tolerance": 0.0001,
        "c0_method": "log_back_extrap",
        "output_pred_variants": True,
        "bloq_policy": {  # same as 6.4
            "pre_dose": "zero", "up_leading": "zero",
            "embedded": "missing", "trailing": "exclude",
        },
...
AUC method: linear (Linear Trapezoidal Linear Interpolation)
...
> 모든 📋 TODO 항목들이 WNL 5.3/6.4/8.3 매뉴얼 직접 검증 완료 (2026-05). 변경 사항은 CHANGELOG.md "Behavior change (version-aware)" 섹션에 기록됨.
